				...
				if (isset ($_POST["procesar"]) )
				{
					...
					while(!feof($file))
					{
						...
						if ($contador == 1) ///INSERTO EL ENCABEZADO
						{
							...
							$_numero_nuc_recaudacion_cabeza = substr($lectura_linea,4,13);
							//$my_date = date('m/d/y', strtotime($date));
							$_fecha_creacion_recaudacion_cabeza = substr($lectura_linea,17,4) .".". substr($lectura_linea,21,2) .".". substr($lectura_linea,23,2);
							$_hora_creacion_recaudacion_cabeza =  substr($lectura_linea,25,2) . ":". substr($lectura_linea,27,2) . ":". substr($lectura_linea,29,2);
							$_cantidad_registros_recaudacion_cabeza =  intval(substr($lectura_linea,31,6));
							$_valor_total_sucres_recaudacion_cabeza =  floatval(substr($lectura_linea,37,13). "." . substr($lectura_linea,50,2));
							$_valor_total_dolares_recaudacion_cabeza =  floatval(substr($lectura_linea,52,13) . ":".substr($lectura_linea,65,2));
